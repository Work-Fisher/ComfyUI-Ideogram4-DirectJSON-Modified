"""Ideogram 4 direct JSON prompt builder, modified.

Modified from ComfyUI-KJNodes' Ideogram 4 prompt builder.
Original project: https://github.com/kijai/ComfyUI-KJNodes
License: GNU GPL v3, see LICENSE in this repository.

Changes in this modified standalone plugin:
- Uses a unique node id/display name so it can coexist with KJNodes.
- Uses import_json as a one-run backend source when the editor has no state yet.
- After the user edits boxes in the visual editor, the serialized editor state
  takes priority so the output JSON reflects manual bbox changes.
"""
...
def _caption_elem_to_box(el, idx=0):
    ...
    box = {
        ...
        "text": el.get("text", ""),
        "desc": el.get("desc", ""),
        "palette": el.get("color_palette", []) if isinstance(el.get("color_palette"), list) else [],
    }
    bb = el.get("bbox")
    if isinstance(bb, list) and len(bb) == 4:
        try:
            ...
            box["w"] = max(0.0, min(1.0, (xmax - xmin) / 1000.0))
            box["h"] = max(0.0, min(1.0, (ymax - ymin) / 1000.0))
            return box
        except (TypeError, ValueError):
            pass

    # No bbox: show a small placeholder in the editor, but keep it flagged so
    # re-export omits the bbox just like the original KJNodes node.
    k = idx % 6
    box["x"] = 0.03 + k * 0.035
    box["y"] = 0.03 + k * 0.035
    box["w"] = 0.22
...
def _boxes_from_caption(cap):
    cd = cap.get("compositional_deconstruction") if isinstance(cap, dict) else None
    elements = cd.get("elements") if isinstance(cd, dict) else None
    if not isinstance(elements, list):
        return []
    return [box for box in (_caption_elem_to_box(el, i) for i, el in enumerate(elements)) if box]


def _bbox_output(boxes, width, height):
    bbox_dicts = []
    for box in boxes:
        if not isinstance(box, dict) or box.get("nobbox"):
            continue
...
class Ideogram4DirectJSONBuilderModified(io.ComfyNode):
    @classmethod
...
the canvas aspect ratio.""",
    ...
    def execute(cls, width, height, background, style,
        ...
        if should_import:
            ...
            ui = {"dims": [width, height], "caption": [_dumps(import_cap)]}
            return io.NodeOutput(_dumps(import_cap), preview, _bbox_output(boxes, width, height), width, height, ui=ui)
        ...
        bboxes_out = _bbox_output(boxes, width, height)

        # ui: send the resolved width/height so the editor canvas can follow connected
        # inputs; import_json (if wired) loads into the editor.
